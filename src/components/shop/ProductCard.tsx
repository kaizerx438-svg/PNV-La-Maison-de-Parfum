...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const handleWishlist = async (e: React.MouseEvent) => {
    ...
    if (res.status === 401) {
      window.location.href = "/login?redirect=/catalogue";
      return;
    }
    ...
    toast.success(
      data.action === "added" ? "Ajoute a la wishlist" : "Retire de la wishlist"
    );
    router.refresh();
  };
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute top-3 right-3 z-10 p-1.5 transition-opacity duration-300 cursor-pointer"
      ...
      {!isOutOfStock && (
        ...
        >
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 text-[10px] tracking-[0.25em] uppercase transition-all duration-300 hover:bg-[#6B1A2A] hover:text-[#F5EFE6] cursor-pointer"
            style={{
      ...
      <div className="mt-4 space-y-1.5 px-1">
        ...
        >
          {categoryLabels[product.category.slug.toUpperCase().replace(/-/g, "_")] ??
            product.category.name}
        </p>
        <h3
          className="text-sm font-light truncate"
          style={{
            color: isOutOfStock ? "rgba(13,13,13,0.4)" : "#0D0D0D",
        ...
        <div className="flex items-center gap-2">
          {isOutOfStock ? (
            <span
              className="text-xs"
              style={{ color: "rgba(13,13,13,0.35)", fontFamily: "Georgia, serif" }}
            >
              Indisponible
            </span>
          ) : discountedPrice ? (
